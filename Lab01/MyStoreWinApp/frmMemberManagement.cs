...
namespace MyStoreWinApp
{
    ...
    public partial class frmMemberManagement : Form
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                var result = _memberRepository.DeleteMemberAsync(_rowIndex+1).Result;
                if (result == 1)
                    LoadMemberList();
            }
            catch (Exception ex)
        ...
        private void rdRole_CheckedChanged(object sender, EventArgs e)
        {
            if (rdRole.Checked)
            {
                _searchBy = SearchBy.ByRole;
            }
        }
        ...
        private void rdCity_CheckedChanged(object sender, EventArgs e)
        {
            if (rdCity.Checked)
            {
                _searchBy = SearchBy.ByCity;
            }
        }

        private void rdCountry_CheckedChanged(object sender, EventArgs e)
        {
            if (rdCountry.Checked)
            {
                _searchBy = SearchBy.ByCountry;
            }
        }
    }
}
